  ...
  <div class="edit-container">
          ...
          >
            <div class="addr-info">
              <div class="addr-top-row">
                ...
                <el-tag v-if="addr.isDefault" type="success" size="small" effect="dark" round>기본</el-tag>
              </div>
              <p class="addr-text">[{{ addr.zipCode }}] {{ addr.roadAddress }} {{ addr.detailAddress }}</p>
    ...
    <el-dialog v-model="dialogVisible" title="새 배송지 추가" width="500px">
      <el-form :model="newAddr" label-position="top">
        <el-form-item label="수령인"><el-input v-model="newAddr.recipient" /></el-form-item>
        ...
        <el-form-item label="연락처"><el-input v-model="newAddr.phone" /></el-form-item>
        <el-form-item><el-checkbox v-model="newAddr.isDefault">기본 배송지로 설정</el-checkbox></el-form-item>
      </el-form>
      <template #footer>
...
const pwdRules = {
  current: [{ required: true, message: '현재 비밀번호를 입력하세요', trigger: 'blur' }],
  new: [{ required: true, message: '새 비밀번호를 입력하세요', trigger: 'blur' }],
  confirm: [{ required: true, message: '비밀번호 확인을 입력하세요', trigger: 'blur' }]
}

// 3. 배송지 정보
const addressList = ref([
  { recipient: '김자바', zipCode: '13529', roadAddress: '판교역로 166', detailAddress: 'A동', phone: '010-1234-5678', isDefault: true },
  { recipient: '회사', zipCode: '04524', roadAddress: '세종대로 110', detailAddress: '8층', phone: '010-9876-5432', isDefault: false }
])
const dialogVisible = ref(false)
const newAddr = reactive({ recipient: '', zipCode: '', roadAddress: '', detailAddress: '', phone: '', isDefault: false })


// --- Logic Methods (Controller 분리 개념) ---

// [로직 1] 기본 정보만 저장 (UPDATE UserProfile)
...
// Utils
function formatPhone(v: string) { return v.replace(/[^0-9]/g, '').replace(/^(\d{2,3})(\d{3,4})(\d{4})$/, `$1-$2-$3`) }
function goBack() {
  router.push('/profile')
}
function openAddressModal() {
  Object.assign(newAddr, { recipient: '', zipCode: '', roadAddress: '', detailAddress: '', phone: '', isDefault: false });
  dialogVisible.value = true
}
function mockAddressSearch() { newAddr.zipCode = '12345'; newAddr.roadAddress = '테스트 로드 123'; }
const pwdError = computed(() => (pwdForm.confirm && pwdForm.new !== pwdForm.confirm) ? '불일치' : '')

</script>

<style scoped>
.edit-container { max-width: 720px; margin: 0 auto; padding-bottom: 40px; color:#333; }
.page-header { padding: 20px 0; }
.content-wrapper { display: flex; flex-direction: column; gap: 24px; }

/* 카드 스타일 */
.section-card { border-radius: 8px; }
.card-header { display: flex; justify-content: space-between; align-items: center; font-weight: 700; font-size: 16px; }

/* 배송지 아이템 스타일 */
.address-item { border: 1px solid #eee; border-radius: 8px; padding: 16px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; }
.address-item.active-card { border-color: #67c23a; background-color: #f0f9eb; }
.addr-top-row { display: flex; align-items: center; gap:8px; margin-bottom:6px; }
...
.error-text { color: #f56c6c; font-size: 12px; margin-top: 4px; display: block; }
</style>
